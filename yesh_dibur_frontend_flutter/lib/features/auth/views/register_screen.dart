...
class _RegisterScreenState extends ConsumerState<RegisterScreen> {
  final _emailController = TextEditingController(text: 'reuven.test@example.com');
  final _passwordController = TextEditingController(text: '123456');
  final _nameController = TextEditingController(text: 'ראובן');
...
void _submit() async {
    ...
    final request = RegisterRequest(
      ...
      email: _emailController.text, // זכור לשנות את המייל בבדיקה!
      phone: _phoneController.text,
      // חובה להמיר ל-UTC כדי שה-ISO יכלול 'Z' בסוף ויעבור את ה-Zod
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('בדיקת הרשמה')),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(16),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            TextField(controller: _emailController, decoration: const InputDecoration(labelText: 'אימייל')),
            const SizedBox(height: 10),
            TextField(controller: _passwordController, decoration: const InputDecoration(labelText: 'סיסמה')),
            const SizedBox(height: 10),
            TextField(controller: _nameController, decoration: const InputDecoration(labelText: 'שם מלא (2-50 תווים)')),
            const SizedBox(height: 10),
            TextField(controller: _phoneController, decoration: const InputDecoration(labelText: 'טלפון')),
            ...
            ElevatedButton(
              ...
              child: authState.isLoading 
                ...
                : const Text('הרשמה ומעבר לשרת', style: TextStyle(fontSize: 18)),
            )
          ],
        ),
      ),
